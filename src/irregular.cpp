		...
		uint64_t recvlimit, double &fraction)
{
	...
	for (int i = 0; i < n; i++)
		bigsendbytes[proclist[i]] += sizes[i];

	//error return if any proc sending > INTMAX to a single proc
	uint64_t maxsend = 0;
	for (int i = 0; i < nprocs; i++)
		maxsend = MAX(maxsend, bigsendbytes[i]);
...
void Irregular::exchange_all2all(
		int 	n,
		int 	*proclist,
		char 	**ptrs,
		int 	*sizes,
...
void Irregular::exchange_custom(
		int 	n,
		int 	*indices,
		char 	**ptrs,
		int 	*sizes,
