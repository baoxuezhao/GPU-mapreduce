...
#define QMRMPI_FPATH QUOTEME(MRMPI_FPATH)
	...
	if (sizeof(uint64_t) != 8)
		error->all("Not compiled for 8-byte integers");
...
uint64_t MapReduce::aggregate(int (*hash)(char *, int))
{
	...
	if (kv == NULL)
		error->all("Cannot aggregate without KeyValue");
	...
	for (int ipage = 0; ipage < maxpage; ipage++)
	{
		...
		while (!done)
		{
			...
			while (1)
			{
				nkey_recv = irregular->setup(stop-start,
						&proclist[start],
						&kvsizes[start],
						&reorder[start],
						twopage,
	...
	printf("time consumed by exchange is %d, %f\n", me, commtime);
...
{
	if (kv == NULL)
		error->all("Cannot collate without KeyValue");
...
{
	if (kv == NULL)
		error->all("Cannot convert without KeyValue");
	if (timer)
		start_timer();
	if (verbosity)
		file_stats(0);
		...
		int 	addflag)
{
	...
	if (selfflag)
		MPI_Allreduce(&nfile,&mapfilecount,1,MPI_INT,MPI_SUM,comm);
	else
		mapfilecount = nfile;

	if(me == 0)
		printf("nproc and #map task is %d, %d\n", nprocs, mapfilecount);
...
uint64_t MapReduce::reduce(
		void (*appreduce)(char *, int, char *, int, int *, KeyValue *, void *),
		void *appptr)
{
	if (kmv == NULL)
		error->all("Cannot reduce without KeyMultiValue");
...
int MapReduce::multivalue_block(
		int 	iblock,
		char 	**pmultivalue,
		int 	**pvaluesizes)
{
...
/* ---------------------------------------------------------------------- */
//can be paralleled using GPU CUDA
...
char *MapReduce::mem_request(int n, uint64_t &size, int &tag)
{
	...
	if (i == npage)
	{
		for (i = 0; i < npage; i++)
		{
			...
			for (j = i+1; j < i+n; j++)
				if (j >= npage || memusage[j] || memcount[j])
					ok = 0;
